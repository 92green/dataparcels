...
* Finally `.spreadInput()` is used to provide the `value` and `onChange` props to the `input` elements.
...
This example also derives the length of the word, but this time it stores it in meta. It also uses an [asNode updater](/api/ParcelNode), an advanced editing feature of dataparcels. The shape updater's syntax can look a little strange, but it allows for powerful manipulations of the shape of a value.
