...
The useParcelForm function is a React hook. Its job is to make submittable forms easy to build, by combining [useParcelState](/api/useParcelState) and [useParcelBuffer](/api/useParcelBuffer) together. It provides a parcel stored in state and a buffer to store unsaved changes, and also handles how the parcel responds to changes in React props.
...
The useParcelForm hook actually holds *two* Parcels in state:
...
```flow
rekey?: ParcelRekeyFunction // optional
```

Applies a [rekey](/api/rekey) function TODOOOO. [See an example here](/ui-behaviour#Keeping-data-in-sync-during-editing).

If the rekey config doesn't need to change after initial mount, it can be returned from a function. The function will only be called on initial mount and the rekey will be cached from then on.
