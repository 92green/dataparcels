...
```flow
validation(validationRuleMap: ValidationRuleMap) => ParcelValueUpdater;
...
The `validation` function returns a `ParcelValueUpdater` function, which is most suitable to be passed into [useParcelForm.beforeUpdate](/api/useParcelForm#beforeUpdate), [useParcelBuffer.beforeUpdate](/api/useParcelBuffer#beforeUpdate) or [useParcelState.beforeUpdate](/api/useParcelState#beforeUpdate).
