...
// updates value - only to be used if shape doesn't change
map(updater: ParcelUpdater): Parcel

// updates shape, including meta
map(asNodes(updater: ParcelUpdater)): Parcel
map(asShape(updater: ParcelUpdater)): Parcel
```
...
This moves the child Parcel at `keyA` to the position of `keyB`.
...
// updates shape, including meta
modifyDown(asNodes(updater: ParcelUpdater)): Parcel
modifyDown(asShape(updater: ParcelUpdater)): Parcel
```
...
// updates shape, including meta
modifyUp(asNodes(updater: ParcelUpdater)): Parcel
modifyUp(asShape(updater: ParcelUpdater)): Parcel
```
