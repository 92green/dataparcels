...
Calling `set()` with one argument will trigger a change that replaces the current value in the Parcel with the `value` provided. This is equivalent to calling [onChange()](#onChange()).
...
update(updateShape(shapeUpdater: ParcelShapeUpdater)): Parcel
update(key: string|number, updateShape(updater: ParcelValueUpdater)): void // only on ParentParcels, will update a child
...
// updates shape, including meta
updateIn(keyPath: Array<string|number>, updateShape(shapeUpdater: ParcelShapeUpdater)): void // only on ParentParcels
type ParcelShapeUpdater = (parcelShape: ParcelShape) => any;
```
...
// updates shape, including meta
map(updateShape(shapeUpdater: ParcelShapeUpdater)): void // only on ParentParcels
type ParcelShapeUpdater = (parcelShape: ParcelShape) => any;
```
...
// updates shape, including meta
modifyDown(updateShape(shapeUpdater: ParcelShapeUpdater)): Parcel
type ParcelShapeUpdater = (parcelShape: ParcelShape) => any;
```
...
// updates shape, including meta
modifyUp(updateShape(shapeUpdater: ParcelShapeUpdater)): Parcel
