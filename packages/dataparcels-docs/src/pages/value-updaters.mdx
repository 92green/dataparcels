...
Use the [updateShape](/api/updateShape) function to allow the shape of data to be safely altered. It will provide you with your Parcel's data wrapped in a [ParcelShape](/api/ParcelShape) which you can then manipulate and return the result.

Shape updaters look something like this:

```js
.modifyUp(updateShape(parcelShape => {
